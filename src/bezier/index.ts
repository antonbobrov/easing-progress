...
import { TEasingBezier } from '../types';

export function easingByBezier(progress: number, bezier: TEasingBezier) {
...
function bezierCalc(progress: number, x1: number, x2: number) {
  return (
    ((bezierA(x1, x2) * progress + bezierB(x1, x2)) * progress + bezierC(x1)) *
    progress
  );
}

function bezierA(x1: number, x2: number) {
  return 1.0 - 3.0 * x2 + 3.0 * x1;
}

function bezierB(x1: number, x2: number) {
  return 3.0 * x2 - 6.0 * x1;
}

function bezierC(x1: number) {
  return 3.0 * x1;
}

function bezierX(bezier: number[], progress: number, val: number[]) {
...
function bezierSlope(progress: number, x1: number, x2: number) {
  return (
    3.0 * bezierA(x1, x2) * progress * progress +
